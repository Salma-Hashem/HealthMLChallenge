...
parse_providers is split into two phases:
  ...
  Phase 2 — _build_provider(block, provider_id, start_dept_id)
      Constructs a Provider object from an already-isolated block.
...
# Path to the patient seed data file — edit this file to add/change patients.
...
def _extract_provider_blocks(lines: List[str]) -> List[List[str]]:
    ...
    No model objects are constructed here — that happens in Phase 2.
    Blocks are closed when the *next* provider header is seen, not at
    end-of-loop, avoiding trailing-save bugs.
    ...
    # Capture the final block — the last provider has no following header to close it.
    if current_block:
        blocks.append(current_block)
...
def _build_provider(
    ...
    """Phase 2: Construct a Provider model from an isolated line block.
...
def load_patient_data() -> Tuple[List[Patient], List[Appointment]]:
    """Load patients and appointment history from data/patients.json.

    Edit data/patients.json directly to add, remove, or update seed patients —
    no Python changes required.
    """
    with open(_PATIENTS_FILE) as fh:
        data = json.load(fh)
    ...
    patients = [
        Patient(
            ...
            referred_providers=[
                ...
                for r in p.get("referred_providers", [])
            ],
        )
    ...
    appointments = [
        Appointment(
            ...
            time=time.fromisoformat(a["time"]),
            status=a["status"],
        )
